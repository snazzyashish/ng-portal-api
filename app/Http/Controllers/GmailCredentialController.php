...
class GmailCredentialController extends Controller
{
    ...
    public function save(Request $req){
        ...
        foreach($modifiedRecords as $value){
            foreach($value as $key => $val ){
                $model = GmailCredential::where('delete_flg',0)->where('id',$value['id'])->first();
                $model->$key = $val;
    ...
    public function list(Request $req){
        ...
        if($sql_where!=''){
            $totalRecords = sizeOf(DB::select('select * from gmail_credentials '.$sql_where.''));
        }else{
            $totalRecords = GmailCredential::where('delete_flg',0)->orderBy('id', 'desc')->count();
        }
        ...
        if($sql_where !=''){
            // $records = DB::select('select * from gmail_credentials '.$sql_where);
            $records = DB::select('select * from gmail_credentials '.$sql_where.' ORDER BY ID DESC');
            
        }else{
            $records = GmailCredential::where('delete_flg',0)->orderBy('id', 'desc')->get();
        }
    ...
    public function delete(Request $req){
        ...
        foreach($deletedRecords as $value){
            $model = GmailCredential::where('delete_flg',0)->where('id',$value)->first();
            $model->delete_flg = 1;
            $saved = $model->save();
        }
