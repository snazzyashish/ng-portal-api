...
class StoreBalanceController extends Controller
{
    ...
    public function list(Request $req){
        ...
        if($sql_where!=''){
            $totalRecords = sizeOf(DB::select('select * from store_balances '.$sql_where.''));
        }else{
            $totalRecords = StoreBalance::where('delete_flg',0)->orderBy('id', 'asc')->count();
        }
        ...
        if($sql_where !=''){
            $credentials = DB::select('select * from store_balances '.$sql_where.'');
            
        }else{
            $credentials = StoreBalance::where('delete_flg',0)->orderBy('id', 'asc')->get();
        }
    ...
    public function save(Request $req){
        ...
        foreach($modifiedRecords as $value){
            foreach($value as $key => $val ){
                $model = StoreBalance::where('delete_flg',0)->where('id',$value['id'])->first();
                $model->$key = $val;
                // $mode->updated_by = $req->user_id;
    ...
    public function delete(Request $req){
        ...
        foreach($deletedRecords as $value){
            $model = StoreBalance::where('delete_flg',0)->where('id',$value)->first();
            $model->delete_flg = 1;
