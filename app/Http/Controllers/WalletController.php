...
class WalletController extends Controller
{
    ...
    public function save(Request $req){
        ...
        foreach($modifiedRecords as $value){
            foreach($value as $key => $val ){
                $model = Wallet::where('delete_flg',0)->where('id',$value['id'])->first();
                $model->$key = $val;
                $saved = $model->save();
            }
        }
    ...
    public function list(Request $req){
        ...
        if($sql_where!=''){
            $totalRecords = sizeOf(DB::select('select * from wallets '.$sql_where.''));
        }else{
            $totalRecords = Wallet::where('delete_flg',0)->orderBy('id', 'desc')->count();
        }
        ...
        if($sql_where !=''){
            // $wallets = DB::select('select * from wallets '.$sql_where);
            $wallets = DB::select('select * from wallets '.$sql_where.' ORDER BY ID DESC');
            
        }else{
            $wallets = Wallet::where('delete_flg',0)->orderBy('id', 'desc')->get();
        }
    ...
    public function delete(Request $req){
        ...
        foreach($deletedRecords as $value){
            $model = Wallet::where('delete_flg',0)->where('id',$value)->first();
            $model->delete_flg = 1;
            $saved = $model->save();
        }
